...
int	ft_printf(const char *format, ...)
{
	...
	while (*format != '\0')
	{
		...
		//*Asociation of unary operation is form R to L
		else
...
/* TYPE PROMOTION
Happends implicitly,
	in some functions. You can also do it explicitly by casting
...
This means the objects of type char or short int are promoted to either int or unsigned int,
	as appropiate,
and float are promoted to double.
So if the caller passes a char as an optional argument,
	it is promoted to an int, and the function can access it
with	va_arg(ap, int)
